...
def _tower_loss(network_fn, images, labels, is_cross = True, reuse=False, is_training=False):
    ...
    for i in range(FLAGS.num_networks):
        ...
        offset2["{0}".format(i)] = net_endpoints["{0}".format(i)]['offset2']
        offset3["{0}".format(i)] = net_endpoints["{0}".format(i)]['offset3']

    return net_pred, offset2, offset3
...
def train():
    ...
    with tf.Graph().as_default():
        ...
        ##############################################################            
        train_image_batch, train_label_batch = utils.get_image_label_batch(FLAGS, shuffle=True, name='train3')
        test_image_batch, test_label_batch = utils.get_image_label_batch(FLAGS, shuffle=False, name='test3')
        train_x = train_image_batch[:,:,:,0:3]
        train_y = tf.cast((tf.squeeze(train_image_batch[:,:,:,3])+1.0)*0.5, tf.int32)
        train_y = 1-tf.one_hot(train_y, depth = 2, axis=-1)
                            
        test_x = test_image_batch[:,:,:,0:3]
        test_y = tf.cast((tf.squeeze(test_image_batch[:,:,:,3])+1.0)*0.5, tf.int32)
        test_y = 1-tf.one_hot(test_y, depth = 2, axis=-1) 
        ...
        with tf.name_scope('tower') as scope:
            with tf.variable_scope(tf.get_variable_scope()):
                net_pred, _, _ = _tower_loss(network_fn, train_x, train_y, is_cross = True, reuse=False, is_training=False)
                ...
                test_net_pred, offset2, offset3 = _tower_loss(network_fn, test_x, test_y, is_cross = True, reuse=True, is_training=False)


                truth = tf.argmax(train_y, axis=-1)
                test_truth = 1-tf.argmax(test_y, axis=-1)
                ...
                test_predictions["{0}".format(0)] = 1-tf.argmax(test_net_pred["{0}".format(0)], axis=-1)
                test_predictions["{0}".format(1)] = tf.argmax(test_net_pred["{0}".format(1)], axis=-1)
                ...
                test_precision["{0}".format(1)] = tf.reduce_mean(tf.to_float(tf.equal(test_predictions["{0}".format(1)], test_truth)))
        ...
        load_fn = slim.assign_from_checkpoint_fn(os.path.join(FLAGS.checkpoint_dir, 'model.ckpt-80'),tf.global_variables(),ignore_missing_vars=True)
        ...
        for batch_idx in range(batch_count):
            #for i in range(FLAGS.num_networks):            
            testpred, test_gt, test_offset2, test_offset3, Test_x = sess.run([test_predictions, test_truth, offset2, offset3, test_x])
            for index in range(test_truth.shape[0]):
                ...
                scipy.misc.imsave(os.path.join(FLAGS.saliency_map, str(batch_idx)+'_'+str(index)+'test_pred1.jpg'), make_png(testpred["{0}".format(1)][index,:,:]))

                if index == 0:
                    #test_offset2 = np.squeeze(test_offset2["{0}".format(1)][index,:,:])
                    test_offset3 = np.squeeze(test_offset3["{0}".format(1)][index,:,:])
                    '''scipy.misc.imsave(os.path.join(FLAGS.saliency_map, str(batch_idx)+'_'+str(index)+'test_offset21.jpg'), make_png(test_offset2))
                    scipy.misc.imsave(os.path.join(FLAGS.saliency_map, str(batch_idx)+'_'+str(index)+'test_offset31.jpg'), make_png(test_offset3))'''
                    #test_offset2 = cv2.applyColorMap(make_png1(test_offset2, test_gt[index,:,:]).astype('uint8'), 2)
                    test_offset3 = cv2.applyColorMap(make_png1(test_offset3, test_gt[index,:,:]).astype('uint8'), 2)
                    #scipy.misc.imsave(os.path.join(FLAGS.saliency_map, str(batch_idx)+'_'+str(index)+'test_offset2.jpg'), test_offset2)
                    scipy.misc.imsave(os.path.join(FLAGS.saliency_map, str(batch_idx)+'_'+str(index)+'test_offset3.jpg'), test_offset3)

            
            testpred0 = np.int32(np.reshape(testpred["{0}".format(0)],[-1]))
            testpred1 = np.int32(np.reshape(testpred["{0}".format(1)],[-1]))
            test_gt0 = np.int32(np.reshape(test_gt,[-1]))
            test_gt1 = np.int32(test_gt0)
            ...
            tn1, fp1, fn1, tp1 = confusion_matrix(test_gt1,testpred1).ravel()

            net0_acc = (tp0 + tn0)/(tp0 + fp0 + tn0 +fn0)
            net1_acc = (tp1 + tn1)/(tp1 + fp1 + tn1 +fn1)
            net0_sen = tp0/(tp0+fn0)
            net1_sen = tp1/(tp1+fn1)
            net0_spe = tn0/(tn0+fp0)
            net1_spe = tn1/(tn1+fp1)
            net0_DI = 2*tp0/(2*tp0+fn0+fp0)
            net1_DI = 2*tp1/(2*tp1+fn1+fp1)
            net0_JA = tp0/(tp0+fn0+fp0)
            net1_JA = tp1/(tp1+fn1+fp1)
            acc0 += net0_acc
            acc1 += net1_acc
            sen0 += net0_sen
            sen1 += net1_sen
            ...
            format_str1 = 'test_net1: %.6f     %.6f     %.6f     %.6f     %.6f'
            print(format_str1 % (net1_acc,  net1_DI, net1_JA, net1_sen, net1_spe))         
            infile.write('test  :acc     dice     jac     sen     spe\n')
            infile.write(format_str % (net0_acc, net0_DI, net0_JA, net0_sen, net0_spe) + ' \n')
            infile.write(format_str1 % (net1_acc,  net1_DI, net1_JA, net1_sen, net1_spe) + ' \n')
        acc0 = acc0/batch_count
        acc1 = acc1/batch_count
        sen0 = sen0/batch_count
        sen1 = sen1/batch_count
        spe0 = spe0/batch_count
        spe1 = spe1/batch_count
        di0 = di0/batch_count
        di1 = di1/batch_count
        ja0 = ja0/batch_count
        ja1 = ja1/batch_count
        print('test_mean  :acc0     dice0     jac0     sen0     spe0      acc1     dice1     jac1     sen1     spe1')
        format_str1 = 'test: acc0: %.6f     %.6f     %.6f     %.6f     %.6f       %.6f     %.6f     %.6f     %.6f     %.6f'
        print(format_str1 % (acc0,  di0, ja0, sen0, spe0, acc1,  di1, ja1, sen1, spe1)) 
        infile.write('test_mean  :acc0     dice0     jac0     sen0     spe0      acc1     dice1     jac1     sen1     spe1\n')
        infile.write(format_str1 % (acc0,  di0, ja0, sen0, spe0, acc1,  di1, ja1, sen1, spe1) + ' \n\n\n')
