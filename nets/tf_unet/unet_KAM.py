        ...
        for layer in range(num_layers - 2, -1, -1):
            with tf.variable_scope("up_conv_{}".format(str(layer)), reuse=reuse):
                ...
                if layer != 0 and layer != 1:
                    ...
                    print('h_deconv',h_deconv)
                    print('dw_h_convs[layer]',dw_h_convs[layer])
                    h_deconv_concat, offset1 = adaptive_deform_con2v(h_deconv, dw_h_convs[layer], features// 2, kernel_size=3, stride=1, trainable=trainable, name='adaptive_conv', reuse=reuse)
                    end_points['offset'] = offset1
